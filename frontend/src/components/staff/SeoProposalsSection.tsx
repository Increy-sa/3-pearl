...
import { CheckCircle2, AlertCircle, Plus, Minus, Globe } from 'lucide-react';
    ...
    fetch(`${API}/api/tickets/${ticket.id}/seo-proposals`, { headers })
      ...
      .then(d => {
        if (d && d.id) {
          ...
          setDomains(dm.length >= 3 ? dm : [...dm, ...Array(3 - dm.length).fill('')]);
        }
      }).catch(() => {});
  }, [ticket.id]);

  // Save proposals (no send logic — transfer handled by FlexibleTransferSection)
    ...
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-bold text-indigo-600 uppercase tracking-wider flex items-center gap-2">
          <Globe className="w-3.5 h-3.5" /> مقترحات الدومين
        </h3>
        ...
        {domains.map((d, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <input type="text" value={d} onChange={e => { const c = [...domains]; c[i] = e.target.value; setDomains(c); }}
              disabled={!canEdit} placeholder={`الدومين المقترح ${i + 1}`}
      ...
      {/* AM Review — approve/revision only, no send buttons */}
      ...
      {/* Approved info — visible to all */}
      {status === 'CLIENT_APPROVED' && proposal && (
