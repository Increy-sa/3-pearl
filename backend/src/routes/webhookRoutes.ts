...
// ─── Webhook Static Token (loaded from .env) ─────────────────────────────
// Set ADTOPIA_WEBHOOK_SECRET in your .env file.
// The server will throw at startup if this variable is missing.
const WEBHOOK_STATIC_TOKEN = process.env.ADTOPIA_WEBHOOK_SECRET;
if (!WEBHOOK_STATIC_TOKEN) {
  throw new Error('FATAL: ADTOPIA_WEBHOOK_SECRET is not set in the environment.');
}

// ────────────────────────────────────────────────────────────────────────────
// POST /api/webhooks/dotmedia
//
// Receives a new lead/order and provisions the user + ticket.
//
// Expected payload:
// {
//   "id": 17,
...
router.post('/threepearl', async (req, res) => {
  ...
  if (!incomingToken || incomingToken !== WEBHOOK_STATIC_TOKEN) {
  ...
  if (!order || !VALID_STATUSES.includes(order.status)) {
    console.warn(`[webhook] ⚠️  Order not successful. Status: "${order?.status}"`);
  ...
  try {
    ...
    console.log(`[webhook] ✅ User upserted — id: ${user.id}, email: ${user.email}`);
    ...
    const clientData = {
      ...
      industry: 'غير محدد',
      description: `طلب وارد من Dot Media — ${serviceTitle}`,
    ...
    const ticket = await prisma.ticket.create({
      data: {
        ...
        clientId: client.id,
        customerId: user.id,
        // Store order context in staffNotes for the Account Manager
        staffNotes: [
          `📦 خدمة: ${serviceTitle}`,
    ...
    console.log(`[webhook] 🎫 Ticket created — id: ${ticket.id}, stage: ${ticket.stage}`);

    // ── 4e. Send Welcome Email with credentials ──────────────────────────
    sendWelcomeEmail(email, password, fullName)
